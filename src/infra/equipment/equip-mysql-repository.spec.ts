...
describe('Equip Mysql Repository', () => {
  ...
  describe('loadEquipByCompanyIdRemoteAccess method tests', () => {
    test('should return a list of equipment with the companyIdRemoteAccess defined', async () => {
      await mockInsertEquip()
      const equipmentList = await sut.loadEquipByCompanyIdRemoteAccess(1)
      expect(equipmentList[0].id).toBeTruthy()
    })

    test('should return an empty list if no records exist', async () => {
      const equipmentList = await sut.loadEquipByCompanyId(0)
      expect(equipmentList.length).toBe(0)
    })
  })
})
