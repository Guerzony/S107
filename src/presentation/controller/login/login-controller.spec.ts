...
describe('Login Controller', () => {
  ...
  test('Should return 401 if invalid credentials are provided', async () => {
    const { sut, authenticationStub } = makeSut()
    jest.spyOn(authenticationStub, 'auth').mockReturnValueOnce(new Promise(resolve => resolve('Error authentication')))
    const httpResponse = await sut.handle(await makeFakeRequestLogin())
